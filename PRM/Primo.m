...
% map=map1';
 source=[5 5];
 goal=[490 490];
 map=im2bw(imread('map2.bmp')); % input map read from a bmp file. for new maps write the file namehere
...
if display, rectangle('Position',[vertex(1,2)-5,vertex(1,1)-5,1,1],'Curvature',[1,1],'FaceColor','r'); end
if display, rectangle('Position',[vertex(2,2)-5,vertex(2,1)-5,1,1],'Curvature',[1,1],'FaceColor','r'); end
tic;
...
for i=1:k+2
    for j=i+1:k+2
        if checkPath(vertex(i,:),vertex(j,:),map);
            ...
            if display, line([vertex(i,2);vertex(j,2)],[vertex(i,1);vertex(j,1)]);pause(0.01); end
        end
    end
end
% if display 
