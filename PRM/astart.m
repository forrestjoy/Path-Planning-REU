...
%%
%imshow(map);
%plot3('position',[1 1 size(map)-1],'edgecolor','k')
vertex=[source;goal]; % source and goal taken as additional vertices in the path planning to ease planning of the robot
if display, plot(vertex(1,2)-5,vertex(1,1)-5,'o','color','g'); end
if display, plot(vertex(2,2)-5,vertex(2,1)-5,'o','color','g'); end
tic;
% waitforbuttonpress;
while length(vertex)<k+2 % iteratively add vertices
    ...
    if feasiblePoint2(x,map1) 
        vertex=[vertex;x]; 
        if display, plot(x(2)-5,x(1)-5,'x','color','r'); end
    end
end
if display 
...
for i=1:k+2
    for j=i+1:k+2
        if checkPath(vertex(i,:),vertex(j,:),map1');
            ...
            if display, plot3([vertex(i,2);vertex(j,2)],[vertex(i,1);vertex(j,1)]);pause(0.01); end
        end
    end
end
if display 
